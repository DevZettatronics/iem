...
include("../admin/libraries/inventory.php");
...
try {
	...
	$data1 = mysqli_fetch_array($result1, MYSQLI_ASSOC);
		...
		$tax_value = $data1['tax_value'];
		$sale_date = date('d/m/Y', strtotime($data1['sale_date']));
		$recargo = $data1['recargo'];
		$payment_date = $data1['payment_date'];// puede ser '2025-08-28' o NULL
	...
	$data2 = mysqli_fetch_array($result2);
		...
		// No s epone el id de vinculacion !== 1 ya que este recibo es exclusivo de alumnos no vinculacion por ello siempre tomara datos
	...
	require_once(dirname(__FILE__) . '/../admin/pdf/html2pdf.class.php');

	// Capturamos el contenido del ticket
	include(dirname('__FILE__') . "/../admin/pdf/documentos/html/tickets3.php");
	...
	try {
		...
		$path_ticket = "../admin/storage/recibos/";
