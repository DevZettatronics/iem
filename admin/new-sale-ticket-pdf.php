...
if (!isset($_SESSION['user_id'])) {
	header("location: index.php"); //incluye
	exit;
}
...
$payment_method = intval($_GET['payment_method']);
$total = intval($_GET['payablePrice']);
$pago = intval($_GET['payment']);
$sale_id = intval($_GET['sale_id']);
$recargo = floatval($_GET['recargo']);
$vinculacion = $_GET['vinculacion'];
$datepago = $_GET['datepago'];

$operacion = '';

if (!empty($datepago)) {
	// Convertir a formato DD/MM/YYYY
	$fecha_formateada = date("d/m/Y", strtotime($datepago));
} else {
	$fecha_formateada = null;
}

//crear transaccion para recibo
mysqli_begin_transaction($con);

try {
	//Consultamos la venta sales
	$consultaS = mysqli_prepare($con,"SELECT * FROM sales  WHERE sale_id = ?");
	if(!$consultaS){ throw new Exception("Error al preparar la consulta en recibo 1"); }

	mysqli_stmt_bind_param($consultaS,"i", $sale_id);
	...
	if ($vinculacion === 1) {
		...
		if(!$consultaP){ throw new Exception("Error al preparar la consulta en recibo 2"); }
	}
		...
		if ($vinculacion !== 1) {
			...
			if (is_numeric($Cuatrimestre)) {
				...
				if (mysqli_num_rows($resultPe) == 0) {
					// El motivo es qeu en la tabla person el usuario en el campo name_periodo no tine numero ES EL PERIODO QUE se relacion en tabla period
			...
			// COn el id_program consultamos nombre y modalidad
			$consultaPro = mysqli_prepare($con,"SELECT name,grade FROM program WHERE id = ?");
	...
	include(dirname(__FILE__) . "/pdf/documentos/html/tickets.php");
	...
	try {
		...
		$path_ticket = "./storage/tickets/";
		if (!file_exists($path_ticket)) {
			if (!mkdir($path_ticket, 0777, true)) {
				throw new Exception("No se pudo crear el directorio de tickets.");
			}
		}
	...
	/* insertar el ticket de pago */
	function insertar_ticket($pdf)
	{
