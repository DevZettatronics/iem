    ...
    if ($_FILES["filename"]) {
        ...
        if ($_FILES["filename"]["name"]) {
            ...
            if (move_uploaded_file($fuente, $carpeta . '/' . $archivonombre)) {
                ...
                //$sqlprincipal = mysqli_query($con, "INSERT INTO recibos (filename,importe,code_user,folio,created_at) VALUES ('" . $archivonombre . "','" . $importe . "','" . $alumn_id . "','" . $folio . "','" . $created_at . "')");
                $sqlprincipal = mysqli_query($con, "INSERT INTO recibos (filename,importe,code_user,folio,created_at, status, id_plan,concepto) VALUES ('" . $archivonombre . "','" . $importe . "','" . $alumn_id . "','" . $folio . "','" . $created_at . "',1, $idpp, $idprod )");

                if ($sqlprincipal) {
                    $messages[] = "Se han creado con exito tus datos en el sistema";
                    Core::redir("./?view=recibos&opt=all");
                    $queryPlanPago = mysqli_query($con, "UPDATE plan_de_pago SET status = 2 WHERE id = $idpp");
                    ...
                    $nombre = PersonData::getByCodeAlumn($alumn_id);//$alumn_id  es el codigo del alumno
                    $name = $nombre->name;
                    $lastname = $nombre->lastname;
                    $fullname = $name . " " . $lastname;
                    $email = $nombre->email; //email del alumno 

                //    $insert_tabla_pago = mysqli_query($con, "INSERT INTO pagos (name, description, total, tipo_pago, order_id, date_created, email, matricula, status, idPerson, id_plan) VALUES ('$fullname', '$idprod', '$importe', 'DEPOSITO', '$folio', now(), '$email', '$alumn_id', '1', '$idPerson', '$idpp');");
                } else {
