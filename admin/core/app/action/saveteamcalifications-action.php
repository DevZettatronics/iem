...
foreach ($alumns as $a) { // ITERAMOS LOS ALUMNOS
	...
	if ($existCalificacionF->existe == 0) { // SI AUN NO HAY CALIFICACION FINAL SE INSERTA 
		...
		$cF->add();
		 ...
		 // Verificamos si la calificación final ha cambiado antes de actualizar
	
		 $existingObservation = CalificationFinalData::getObservation($a->alumn_id, $_POST["asignation_id"]);
		 if ($existCalificacionF->calificacion != $promedioFinal || $existingObservation->observaciones != $_POST['option']) {
			 // Si la calificación o la observación han cambiado, actualizamos
			//  CalificationFinalData::update_val($a->alumn_id, $_POST["asignation_id"], $promedioFinal, $_POST['option']);
			 echo CalificationFinalData::update_val($a->alumn_id, $_POST["asignation_id"], $promedioFinal,  $_POST['option']);
