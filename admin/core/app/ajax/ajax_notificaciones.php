...
if (isset($_POST['view'])) {
	...
	$query = "SELECT * FROM notificaciones WHERE kind = 1 ORDER BY id DESC LIMIT 3 ";
	...
	if (mysqli_num_rows($result) > 0) {
	   ...
	   ';
		...
		$output .= '
		 <li><a href="./?view=notificaciones&opt=all" class="text-bold text-black">Ver todas las notificaciones</a></li>';
	} else {
		$output .= '
...
//codigo para alert inicial"   // WHERE estado=0 AND id_receptor = $user 
	...
	$status_query = "SELECT * FROM notificaciones WHERE estado=0 AND kind = 1 ";
