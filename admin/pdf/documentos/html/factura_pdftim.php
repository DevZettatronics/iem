...
$sql_tmp = mysqli_query($con, "SELECT * FROM factura where folio=$id_ticket"); // esta tabla debe ser cambiada cuando se agrege el timbrado para la visualizacion ya que dejara de estar en una tabla "temporal" a una tabla fija para tener el registro sin cambios
...
?>
<?php /* PARA AGREGAR EL CODIGO QR A LA FACTURA */
$directory = '../storage/factura/' . $ticket_id . '';
...
<?php //  *****IMPORTANTE***** ESTO AYUDA A RECUPERAR LA INFORMACION DE LOS ARCHIVOS XML ******NO BORRAR***** 
$xml = simplexml_load_file('../storage/factura/' . $ticket_id . '.xml');
$ns = $xml->getNamespaces(true);
$xml->registerXPathNamespace('c', $ns['cfdi']);
$xml->registerXPathNamespace('t', $ns['tfd']);

//  *****IMPORTANTE***** ESTO AYUDA A RECUPERAR LA INFORMACION DE LOS ARCHIVOS XML ******NO BORRAR***** 
//CON LAS SIGUIENTES LINEAS SE LEE LA INFORMACION DEL CFDI PARA PODER IMPRIMIR EN LA FACTURA 
foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Complemento') as $cfdiComprobante) {
...
foreach ($xml->xpath('//cfdi:Comprobante') as $cdfnom) {
    $cfdinocer = $cdfnom['NoCertificado'];
    echo "<br />";
}
...
//UTILIZAR ESTAS VARIABLES PARA IMPRIMIR EN LA FACTURA
foreach ($xml->xpath('//t:TimbreFiscalDigital') as $tfd) {
    $versionS = $tfd['Version'];
    $UUID = $tfd['UUID'];
    $fechaTimbrado = $tfd['FechaTimbrado'];
    $selloCDF = $tfd['SelloCFD'];
...
<body lang=ES-MX>
    ...
    <table class="tabinfoemp">
        ...
        <tr>
            ...
            <b>Código postal:</b><?php echo " (".$customers_cp.") ".$customers_city .", ". $customers_muni .", ". $customers_state ?> <br><br>
    ...
    <table class="tabproducts">
        ...
        <?php
        $sumador_total = 0;
        $sql_c = mysqli_query($con, "SELECT * from products, sale_product where products.product_id=sale_product.product_id and sale_product.sale_id='$id_ticket';");
    ...
    <tr>
        ...
        <tr>
            <td><?php echo wordwrap($versionS . "|" . $UUID . "|" . $rfc_recb . "|" . $selloCDF . "|" . $noCertificadoSAT, 140, " ", TRUE); ?></td>
        </tr>
    </table>

    <table class="tabSHCP2">
        <tr>
            <td rowspan="7" style="padding-top:10px; text-align: center; padding-bottom:10px; background-color:#fff; width:160px;"> 
                <img style="width: 130px;height: 130px;" <?php echo $imagenQR ?> >
            </td> 
        </tr>
    </table>
    ...
    <tr>
        ...
        <tr  style="background-color: #fff;">
            <td><?php echo wordwrap($selloCDF, 110, " ", TRUE); ?></td>
        </tr>
        <tr>
            <td >Sello del SAT</td>
        </tr>
        <tr>
            <td  style="background-color: #fff;"><?php echo wordwrap($selloSAT, 110, " ", TRUE); ?></td>
        </tr>
    </table>
