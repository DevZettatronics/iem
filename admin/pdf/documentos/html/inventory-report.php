<style type="text/css">
	...
	-->
	<?php
	/* llamado para que imprima como usuario */
	include("../config/permisos.php");
	session_start();
	$grupo = $_SESSION['user_group_id'];
	?>
</style>
<page backtop="10mm" backbottom="10mm" backleft="10mm" backright="10mm" style="font-size: 10px; font-family: helvetica" backimg="">
	...
	<div style="width: 20%; text-align: left">
		<img src="<?php echo $logo_url; ?>" style="width: 100px;">
	</div>
	...
	<table class="table-bordered" style="width:100%;" cellspacing=0>
		<tr>
			<th class='top bottom' style="width: 10%;text-align:center">C&oacute;digo</th>
			<th class='top bottom' style="width: 35%;">Producto</th>
			<th class='top bottom' style="width: 15%;">Categoría</th>
			<th class='top bottom' style="width: 10%;text-align:center">Existencias</th>

			<?php if ($grupo == 1 || $grupo == 7) { ?>
				<th class='top bottom' style="width: 10%;text-align:right">Costo</th>

			<?php }
			if ($grupo == 3 || $grupo == 7) {  ?>
				<th class='top bottom' style="width: 10%;text-align:right">Precio Venta</th>
			<?php } ?>
			<?php if ($grupo == 1 || $grupo == 7) { ?>
		...
		while ($row = mysqli_fetch_array($query)) {
			...
			if ($grupo == 3 || $grupo == 7) {
				$precio_venta = $row['selling_price']; // precio venta
			}
			if ($grupo == 1 || $grupo == 7) {
		...
		?>
			<tr>
				...
				<td class='bottom' style="width: 35%;"><?php echo $product_name; ?></td>
				<td class='bottom' style="width: 15%;"><?php echo $category_name; ?></td>
				<td class='bottom' style="width: 10%;text-align:center"><?php echo get_stock($product_id); ?></td>

				<?php if ($grupo == 1 || $grupo == 7) { ?>
					<td class='bottom' style="width: 10%;text-align:right"><?php echo number_format(
																				$buying_price,
																				$currency_format['precision_currency'],
																				$currency_format['decimal_separator'],
																				$currency_format['thousand_separator']
																			); ?></td>
				<?php }
				if ($grupo == 3 || $grupo == 7) {  ?>
